...
{
	//빈 공간이 나올 때까지 다음 회원 객체 주소로 넘어감
	for (member* current_location = head->next; current_location != NULL; current_location->next)
	{
		//입력받은 회원번호와 주소가 가리키는 회원번호가 같다면 현재 주소 반환
		if (current_location->member_number == member_number)
		{
...
{
	cout << "   " << ptr_member->member_number + 1 << "	" << ptr_member->name << "	" <<
		ptr_member->hpnumber << "	" << ptr_member->gender << "	" << ptr_member->date_start << "	"
		<< ptr_member->date_end << "	" << ptr_member->clothes << "	" << ptr_member->trainer << "	"
		<< ptr_member->date_class << "	" << ptr_member->time_class << endl << endl;
}
...
void member_list::add_member()
{
	...
	new_member->clothes = (_clothes == "y") ? "true" : "false"; //삼항연산자, y입력시 true
...
void member_list::rewrite_member()
{
	...
	{
		cout << "해당 회원번호에 해당하는 회원님의 정보가 존재하지 않습니다. " << endl << endl;
	}
...
void member_list::search_member() const
{
	...
	case 1:
		...
		{
			if (this_member->member_number == --member_Classification) 
				member_profile(this_member); //회원 번호가 같으면 프로필(회원 정보)출력
	...
	case 2:
		...
		while (this_member != NULL) //빈 공간이 나올때까지 반복
		{
			if (this_member->name == str) 
				member_profile(this_member); //이름이 같으면 프로필(회원 정보)출력
	...
	default:
		cout << "해당 회원님의 정보가 존재하지 않습니다." << endl << endl;
		return;
	}
}
...
void menu_fitness_management(member_list* member)
{
	...
	menu_fitness_management(member);
}
...
void special_member_list::add_member()
{
	...
	string _name, _hpnumber, _gender, _date_start, _date_end, _clothes, _date_class, _time_class, _trainer;
	int _membership;
	static int _member_number = 0;
	...
	cout << "수업 요일(월, 화, 수, 목, 금, 토요일 중에 입력해주세요.) : ";
	cin >> _date_class;
	
	cout << "수업 시간(06:00 ~ 23:00) : ";
	cin >> _time_class;

	if (member_Classification == 2) _trainer = "조광호(스피닝 담당)";
	else if (member_Classification == 3) _trainer = "이도윤(PT 담당)";
	else _trainer = "김형준(필라테스 담당)";
	...
	new_member->date_start = _date_start;
	new_member->date_end = _date_end;
	new_member->clothes = (_clothes == "y") ? "true" : "false"; //삼항연산자, y 입력시 true
	new_member->date_class = _date_class;
	new_member->time_class = _time_class;
	new_member->trainer = _trainer;
...
void special_member_list::rewrite_member()
{
	int _membership = 0, _member_number = 0;
	string _date_end, _clothes;
	member* current_location = member_location(_member_number); //회원 번호로 회원 정보에 접근하는 포인터변수
	...
	else
	{
		...
		cout << "수업 요일(월, 화, 수, 목, 금, 토요일 중에 입력해주세요.) : ";
		cin >> current_location ->date_class;
